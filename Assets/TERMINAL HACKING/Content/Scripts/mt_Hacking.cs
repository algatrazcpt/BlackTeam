...
public class mt_Hacking : MonoBehaviour
{
    ...
    {
        FindClosestTerminal();
        term = closestTerminal.GetComponent<mt_Terminal>();
        UpdateCursorLock();
    ...
    void LockedStart()
    {
        ...
        footerText.text = "SYSTEM HAS BEEN LOCKED DUE TO MALICOUS ACTIVITY, PLEASE CONTACT A SYSTEM ADMINISTRATOR TO RESET LOGIN";
    }
    ...
    public void ToggleHacking(bool enabled)
    {
        ...
        isHacking = enabled;
        if (enabled) Setup();
        //  if (enabled) Player.DeactivateAll();
